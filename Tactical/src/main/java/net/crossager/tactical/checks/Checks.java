package net.crossager.tactical.checks;
...
public class Checks {
    public static void notLonger(CharSequence str, int length, String name) {
        if (str.length() > length) throw new IllegalStateException(name + " cannot be longer than " + length + "(" + str + ")");
    }
    public static void notEmpty(String str, String name) {
        if (str == null || str.isEmpty() || str.isBlank()) throw new IllegalStateException(name + " cannot be empty (" + str + ")");
    }
