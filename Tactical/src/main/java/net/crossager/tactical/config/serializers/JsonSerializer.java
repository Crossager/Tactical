...
public class JsonSerializer implements TacticalConfigSerializer {
    ...
    private JsonElement serializeChildren(Map<String, TacticalConfigValue> children) {
        ...
        children.forEach((name, object) -> jsonObject.add(name, serializeElement(object)));
        return jsonObject;
    }
    ...
    private JsonElement serializeElement(TacticalConfigValue object) {
        ...
        if (object.isList()) {
            ...
            object.asList().forEach(element -> jsonElements.add(serializeElement(element)));
            return jsonElements;
        }
        JsonObject jsonObject = new JsonObject();
        object.asSection().children().forEach((path, element) -> jsonObject.add(path, serializeElement(element)));
        return jsonObject;
    }
    ...
    public void deserialize(@NotNull String data, @NotNull TacticalConfigSection section) {
        ...
        children.forEach((name, child) -> deserializeChild(name, child, section));
    }

    private void deserializeChild(String path, JsonElement element, TacticalConfigSection section) {
        if (element.isJsonObject()) {
            Map<String, JsonElement> children = childrenOf(element.getAsJsonObject());
            TacticalConfigSection newSection = section.createSection(path);
            children.forEach((name, child) -> deserializeChild(name, child, newSection));
            return;
        }
        if (element.isJsonArray()) {
            TacticalConfigList newList = section.createList(path);
            element.getAsJsonArray().forEach(e -> deserializeElement(e, newList));
            return;
        }
