...
public class XmlSerializer implements TacticalConfigSerializer {
    private static boolean hasWarnedAboutNewline = false;
    public static final List<Character> ALLOWED_NAME_CHARACTERS = List.of('-', '_', '.', '\n');

    private static void newlineWarn() {
        if (!hasWarnedAboutNewline) {
            TacticalAPI.getInstance().getLogger().warning("Newlines (\\n) in xml config will be removed!");
            hasWarnedAboutNewline = true;
        }
    }
    ...
    private Node serializeValue(String name, TacticalConfigValue value) {
        name = checkName(name);
        if (value.isEmpty()) return new Element(name);
        if (value.isNumber()) return new Element(name).text(checkValue(value.asNumber().toString()));
        if (value.isString()) return new Element(name).text(checkValue(value.asString()));
        if (value.isBoolean()) return new Element(name).text(checkValue(String.valueOf(value.asBoolean())));
        if (value.isCharacter()) return new Element(name).text(checkValue(String.valueOf(value.asCharacter())));
        if (value.isList()) {
    ...
    private void deserializeChild(Element element, TacticalConfigSection section) {
        ...
        if (element.attr("type").equals("list")) {
            TacticalConfigList newList = section.createList(element.tagName());
            element.children().forEach(e -> deserializeElement(e, newList));
            return;
        }
        TacticalConfigSection newSection = section.createSection(element.tagName());
