...
public class SimpleTacticalConfigSection extends BaseConfigItem implements TacticalConfigSection {
    ...
    public @NotNull TacticalConfigValue get(@NotNull String path) {
        ...
        if (children.containsKey(str[0]))
            return children.get(str[0]).createSection().get(str[1]);
    ...
    @Override
    public @NotNull TacticalConfigSection createSection(@NotNull String path) {
        checkExists();
        return get(ConfigUtils.checkPath(path)).createSection();
    }

    @Override
    public @NotNull TacticalConfigList createList(@NotNull String path) {
        checkExists();
        return get(ConfigUtils.checkPath(path)).createList();
    }
